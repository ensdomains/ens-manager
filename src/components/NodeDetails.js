...
import { updateForm, switchTab } from '../updaters/nodeDetails'
import {
  watchRegistryEvent,
  watchResolverEvent,
  stopWatching
...
async function handleUpdateOwner(name, currentOwner, newOwner) {
  ...
  if (currentOwner === db.accounts.get(0)) {
    setNewOwner(name, newOwner).then(txId => {
      let sentComponent = (
        <span>
          New owner <TxLink addr={etherscanAddr} txId={txId} /> for {name} sent!
        </span>
      )
      ...
      watchRegistryEvent('Transfer', name, (error, log, event) => {
        ...
        let confirmedComponent = (
          <span>
            New owner <TxLink txId={txId} /> for {name} confirmed!
          </span>
        )
    ...
    setSubnodeOwner(
      ...
      watchRegistryEvent('NewOwner', node, (error, log, event) => {
        ...
        let confirmedComponent = (
          <span>
            New owner <TxLink addr={etherscanAddr} txId={txId} /> for {node}{' '}
            confirmed!
          </span>
        )
        addNotification(confirmedComponent, false)
...
function handleSetResolver(name, newResolver) {
  setResolver(name, newResolver).then(async txId => {
    ...
    let sentComponent = (
      <span>
        New Resolver <TxLink addr={etherscanAddr} txId={txId} /> for {name}{' '}
        sent!
      </span>
    )
    addNotification(sentComponent, false)
...
function handleCheckSubDomain(label, node) {
  ...
  checkSubDomain(label, node)
    ...
    .catch(err => console.error('in catch', err))
}
  ...
  checkSubDomain(subDomain, domain).then(address => {
    if (address !== '0x0000000000000000000000000000000000000000') {
      ...
      createSubDomain(subDomain, domain).then(async ({ owner, txId }) => {
        ...
        let sentComponent = (
          <span>
            New Subdomain <TxLink addr={etherscanAddr} txId={txId} /> for{' '}
            {domain} sent!
          </span>
        )
        addNotification(sentComponent, false)
        watchRegistryEvent('NewOwner', domain, async (error, log, event) => {
          ...
          if (log.args.owner === owner && labelHash === log.args.label) {
            ...
            addNotification(confirmedComponent, false)
            let subDomainObject = await buildSubDomain(
              subDomain,
              domain,
              log.args.owner
            )
            appendSubDomain(subDomainObject)
            event.stopWatching()
          }
        })
      })
    ...
    if (address === '0x0000000000000000000000000000000000000000') {
      ...
      deleteSubDomain(subDomain, domain).then(txId => {
        watchRegistryEvent('NewOwner', domain, async (error, log, event) => {
          ...
          let labelHash = web3.sha3(subDomain)
          if (
            parseInt(log.args.owner, 16) === 0 &&
            log.args.label === labelHash
          ) {
            ...
            let confirmedComponent = (
              <span>
                Delete domain <TxLink addr={etherscanAddr} txId={txId} /> for{' '}
                {subDomain + '.' + domain} confirmed!
              </span>
            )
            addNotification(confirmedComponent, false)
            event.stopWatching()
...
function isOwnerOrParentIsOwner(account, name) {
  ...
  if (parent) {
    return (
      db.accounts.get(0) === getParentNode(name).get('owner') ||
      db.accounts.get(0) === account
    )
  }
...
const Tabs = ({ selectedNode, currentTab }) => {
  ...
  return (
    <div className="tabs">
      <h2
        className={classnames('tab', { current: currentTab === 'nodeDetails' })}
        onClick={() => handleSwitchTab('nodeDetails')}
      >
        Node Details
      </h2>
      <h2
        className={classnames('tab', {
          current: currentTab === 'resolverDetails',
          'has-resolver': hasResolver
        })}
        onClick={handleResolverTabSwitch}
      >
        Resolver Details
      </h2>
    </div>
...
const NodeDetails = ({ selectedNode }) => {
  ...
  checkSubdomain = (
    <div className="input-group">
      <input
        type="text"
        name="subDomain"
        value={db.getIn(['updateForm', 'subDomain'])}
        onChange={e => handleOnChange('subDomain', e.target.value)}
      />
      <button
        onClick={() =>
          handleCheckSubDomain(
            db.getIn(['updateForm', 'subDomain']),
      ...
      <div className="input-group">
        ...
        <button
          onClick={e =>
            handleUpdateOwner(
              getNodeInfo(selectedNode, 'name'),
              getNodeInfo(selectedNode, 'owner'),
              db.getIn(['updateForm', 'newOwner'])
            )
          }
        >
          Update owner
  ...
  if (isOwner(nodeOwner)) {
      ...
      <div className="input-group">
        ...
        <button
          ...
          onClick={() =>
            handleSetResolver(
              getNodeInfo(selectedNode, 'name'),
              db.getIn(['updateForm', 'newResolver'])
            )
          }
        >
          Set Resolver
    ...
    defaultResolver = (
      <button onClick={() => handleSetDefaultResolver()}>
    ...
    createSubDomain = (
      <div className="input-group">
        <input
          type="text"
          name="subDomain"
          value={db.getIn(['updateForm', 'newSubDomain'])}
          onChange={e => handleOnChange('newSubDomain', e.target.value)}
        />
        <button
          onClick={() =>
            handleCreateSubDomain(
              db.getIn(['updateForm', 'newSubDomain']),
              getNodeInfo(selectedNode, 'name')
            )
          }
        >
      ...
      <button
        ...
        onClick={() =>
          handleDeleteSubDomain(
            getNodeInfo(selectedNode, 'label'),
            getNodeInfo(selectedNode, 'node')
          )
        }
      >
        Delete Node
...
export default () => {
  ...
  let renderedContent = (
    <div>Search and select a domain to start managing your domains!</div>
  )
  let tabContent
  let addr
  let content
  ...
  if (selectedNode) {
    switch (currentTab) {
      ...
      case 'resolverDetails':
        ...
        addr = (
          <div className="current-addr info">
            <strong>Address:</strong> {getNodeInfo(selectedNode, 'addr')}
          </div>
        )
        content = (
          <div className="current-content info">
            <strong>Content:</strong> {getNodeInfo(selectedNode, 'content')}
          </div>
        )
        break
    }
      ...
      <div>
        ...
        <div className="info-container">
          <div className="current-node info">
            <strong>Name:</strong> {getNodeInfo(selectedNode, 'name')}
          </div>
          <div className="current-owner info">
            <strong>Owner:</strong> {getNodeInfo(selectedNode, 'owner')}
          </div>
          <div className="current-resolver info">
            <strong>Resolver:</strong> {getNodeInfo(selectedNode, 'resolver')}
          </div>
          {addr}
          {content}
        </div>
  ...
  return <div className="node-info">{renderedContent}</div>
}
